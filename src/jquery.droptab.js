(function($){
	$.fn.droptab = function(options){
		...
		this.addClass(menuClass).before('<div class="'+selectBoxClass+'"><span class="'+selectCurrent+'"></span><span class="'+selectButton+'">Open Button</span></div>');
		...
		droptabSelector.find('.'+selectButton).on('click',function(){
			var button = $(this),
				...
				h = 0;
			if($(this).parent().hasClass(menuOpenClass)){ // closing
				...
				if(activeText){
					setMsg(selectCurrentElement, msgTransitionTime, hiddenMsgClass, activeText);
				}
			}else{ // opening
				droptab.find('li').each(function(){
				...
				if(selectCurrentElement.text() != openMsg){
					setMsg(selectCurrentElement, msgTransitionTime, hiddenMsgClass, openMsg);
				}
			}
		});
		...
		droptab.find('a').on('click',function(e){
			if($(this).closest('.'+menuClass).hasClass(smallMenuClass) && autoClose == true){
				setTimeout(function(){
		...
		function getLoc(breakWidth, loc, droptab){
			...
			if(w < breakWidth){
				...
				if(loc == 'L' || loc == ''){
					...
					loc = 'S';
					droptab.css('height',0).addClass(smallMenuClass).prev().css('display','block');
				}
			}else{
				// Large size
				if(loc == 'S' || loc == ''){
					...
					loc = 'L';
					droptab.css('height','auto').removeClass(smallMenuClass+' '+menuOpenClass).prev().removeClass(menuOpenClass).css('display','none');
				}
			}
			return loc;
		};

		function setMsg(selectCurrentElement, msgTransitionTime, hiddenMsgClass, newMsg) {
			selectCurrentElement.addClass(function(){
				setTimeout(function(){
					selectCurrentElement.empty().text(newMsg).removeClass(hiddenMsgClass);
				}, msgTransitionTime);
		...
		function getActiveTxt(droptab, activeClass, openMsg){
			if(droptab.find('.'+activeClass).length > 0){
				var activeText = droptab.find('.'+activeClass).find('a').text()
				return activeText;
			}else{
				return openMsg
			}
